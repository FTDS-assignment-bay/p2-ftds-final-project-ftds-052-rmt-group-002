"""
Customers router — list, detail, transactions
"""
...
class CustomerListResponse(BaseModel):
    ...
    predicted_clv_90d: Optional[float]
...
CUSTOMER_DETAIL_SELECT = """
    SELECT
        ...
        c.first_seen_date, r.churn_probability, r.risk_segment,
        r.predicted_clv_90d, r.segment_name, r.model_version,
        r.computed_at, r.recency_days, r.frequency, r.monetary_total,
        r.monetary_avg, r.preferred_category, r.preferred_device,
        r.preferred_payment
    FROM latest_rfm r
...
def get_customers_count(
    ...
    """Number of customers, optionally filtered by risk segment."""
...
@router.get("/list", response_model=List[CustomerListResponse])
def get_customer_list(
    risk_segment: Optional[str] = Query(None, description="Filter: high, medium, low"),
    limit: int = Query(200, ge=1, le=1000),
    offset: int = Query(0, ge=0),
):
    """Lightweight customer list for table view."""
    ...
    rows = query(
        ...
        + """
        SELECT
            r.customer_id, c.full_name, r.churn_probability, r.risk_segment,
            r.predicted_clv_90d, r.segment_name, r.recency_days,
            r.frequency, r.monetary_total
        FROM latest_rfm r
        LEFT JOIN dim_customers c ON r.customer_id = c.customer_id
        """
...
def get_customer_detail(customer_id: str):
    """Full churn prediction detail and RFM for a single customer."""
    row = query_one(
        LATEST_RFM_CTE + CUSTOMER_DETAIL_SELECT + "WHERE r.customer_id = %s",
        (customer_id,),
    )
    if not row:
        raise HTTPException(
